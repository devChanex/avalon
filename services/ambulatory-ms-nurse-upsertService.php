...
class ServiceClass
{
    ...
    public function process($data)
    {
        try {
            ...
            $isInsert = empty($data['msnid']);
            $table = "ambulatory_medication_sheet_nurse";
            ...
            if ($isInsert) {
                ...
                $sql = "UPDATE {$table} SET " . implode(", ", $updates) . " WHERE msnid = :msnid";
            }
            ...
            try {
                ...
                // Get the record ID
                $recordId = $isInsert ? $this->conn->lastInsertId() : $data['msnid'];

                // Fetch the saved record
                $query = "SELECT * FROM {$table} WHERE msnid = :id";
                ...
                return [
                    "success" => true,
                    "message" => $isInsert ? "Ambulatory Medicine Sheet record added successfully." : "Ambulatory Medicine Sheet updated added successfully.",
                    "record" => $record,
