...
class ServiceClass
{
    ...
    public function process($data)
    {
        ...
        $sortBy = isset($data['sortBy']) ? $data['sortBy'] : 'medcertid'; // default sort field
        ...
        $searchFields = ["CONCAT('OPD', LPAD(opdcid, 6, '0'))", "CONCAT(last_name, ' ', suffix, ', ', first_name, ' ', middle_name)", "physician", "examined_date", "cert_date"];
        $dynamics = '';
        if (!empty($search)) {
            $orConditions = [];
        ...
        try {
            ...
            $countQuery = "SELECT COUNT(medcertid) as total  FROM medical_certificate a inner join patients b on a.pid=b.id  $dynamics ";
            ...
            $query = "SELECT a.*,CONCAT(last_name, ' ', suffix, ', ', first_name, ' ', middle_name) as fullname,birth_date,contact_number,gender,present_address,patient_no FROM medical_certificate a inner join patients b on a.pid=b.id $dynamics  LIMIT :limit OFFSET :offset";
