...
class ServiceClass
{
    ...
    public function process($data)
    {
        try {
            ...
            $isInsert = empty($data['ampnid']);
            $table = "ambulatory_progress_nurse";
            ...
            if ($isInsert) {
                ...
                $sql = "UPDATE {$table} SET " . implode(", ", $updates) . " WHERE ampnid = :ampnid";
            }
            ...
            try {
                ...
                // Get the record ID
                $recordId = $isInsert ? $this->conn->lastInsertId() : $data['ampnid'];

                // Fetch the saved record
                $query = "SELECT * FROM {$table} WHERE ampnid = :id";
