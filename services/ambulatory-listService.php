...
class ServiceClass
{
    ...
    public function process($data)
    {
        ...
        $searchFields = ["CONCAT('AS', LPAD(amid, 6, '0'))", "CONCAT(last_name, ' ', suffix, ', ', first_name, ' ', middle_name)", "physician", "surgery_date", "caseno"];
        $dynamics = '';
        if (!empty($search)) {
            $orConditions = [];
        ...
        try {
            ...
            $query = "SELECT a.*,CONCAT(last_name, ' ', suffix, ', ', first_name, ' ', middle_name) as fullname,birth_date,contact_number,gender,present_address,patient_no,caseno FROM ambulatory_main a inner join patients b on a.pid=b.id $dynamics  LIMIT :limit OFFSET :offset";
