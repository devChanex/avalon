...
class ServiceClass
{
    ...
    public function process($data)
    {
        try {
            ...
            if (empty($data['pid']) || empty($data['amid'])) {
                return [
                    'success' => false,
            ...
            $isInsert = empty($data['amdataid']);
            $table = "ambulatory_data";
            ...
            if ($isInsert) {
                ...
                $sql = "UPDATE {$table} SET " . implode(", ", $updates) . " WHERE amdataid = :amdataid";
            }
            ...
            try {
                ...
                // Get the record ID
                $recordId = $isInsert ? $this->conn->lastInsertId() : $data['amdataid'];

                // Fetch the saved record
                $query = "SELECT * FROM {$table} WHERE amdataid = :id";
