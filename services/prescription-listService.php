...
class ServiceClass
{
    ...
    public function process($data)
    {
        ...
        $searchFields = ["CONCAT('OPD', LPAD(opdcid, 6, '0'))", "CONCAT(last_name, ' ', suffix, ', ', first_name, ' ', middle_name)", "physician", "prescription_date", "next_appointment"];
        $dynamics = '';
        if (!empty($search)) {
            $orConditions = [];
        ...
        try {
            ...
            $countQuery = "SELECT COUNT(prescriptid) as total  FROM prescription a inner join patients b on a.pid=b.id  $dynamics ";
