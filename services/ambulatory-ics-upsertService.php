...
class ServiceClass
{
    ...
    public function process($data)
    {
        try {
            ...
            $isInsert = empty($data['icsid']);
            $table = "ambulatory_instrument_count";
            ...
            if ($isInsert) {
                ...
                $sql = "UPDATE {$table} SET " . implode(", ", $updates) . " WHERE icsid = :icsid";
            }
            ...
            try {
                ...
                // Get the record ID
                $recordId = $isInsert ? $this->conn->lastInsertId() : $data['icsid'];

                // Fetch the saved record
                $query = "SELECT * FROM {$table} WHERE icsid = :id";
                ...
                return [
                    "success" => true,
                    "message" => $isInsert ? "Ambulatory Instrument Count record added successfully." : "Ambulatory Instrument Count record updated successfully.",
                    "record" => $record,
